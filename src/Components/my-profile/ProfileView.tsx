...
const ProfileView = () => {
  ...
  const skillEndorsements = user?.skills.map((skill) => {
    const count = user?.endorsements.filter((e) => e.skill === skill).length
    return { skill, count }
  })
//  console.log(skillEndorsements)
 
 const {data:endorse,error,isPending}= useQuery({
   queryKey:["getEndorse"],
   queryFn: async ()=>{
     const {data} = await axiosInstance.get(`/endorse/endorsements/${user?._id}`)
     return data
    },
    enabled: !!user
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex gap-2 mb-4 md:mb-0">
                <Button
                  variant="outline"
                  size="sm"
                  className="gap-2 border-[#611f69] text-[#611f69] hover:bg-white/20 hover:text-[#611f69] dark:bg-white/70 dark:border-[#611f69]/70 dark:text-[#83218e]  "
                ...
                <CardContent>
                  <div className="space-y-6 ">
                    {skillEndorsements
                      ?.sort((a, b) => b.count - a.count)
                      .map((item) => (
                        <div key={item.skill} className="space-y-2 border-b border-gray-100 p-2 hover:shadow dark:border-gray-800 ">
